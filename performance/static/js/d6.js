...
D6.dice = function(numDice, callback, callbackData, useImages, buttonLabel) {
	...
	if (buttonLabel != "none") {
		genHtml += "<div id='diceform'><form><input type='button' id='dicebutton' value='" + buttonLabel + "' onclick='D6AnimBuilder.get(\"dice\").reset(); D6AnimBuilder.get(\"dice\").start(); incrementScore()' /></form></div>";
	}
	genHtml += "</div>";
	D6.genHtml = genHtml;
